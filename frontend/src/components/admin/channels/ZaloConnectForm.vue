...
<template>
  <UForm class="space-y-4" id="zalo-connect-form" :schema="zaloConnectSchema" :state="state" @submit="(e: FormSubmitEvent<ZaloConnectSchema>) => emit('submit', e.data)">
    <UFormField label="Bot Token" name="bot_token" required>
      <UInput class="w-full" size="sm" type="password" v-model="state.bot_token" placeholder="123456:abc-xyz" />
    </UFormField>
    <UFormField label="Verify Token" name="verify_token" required>
      <UInput class="w-full" size="sm" v-model="state.verify_token" placeholder="my_verify_token" />
    </UFormField>
    <UAlert color="neutral" variant="soft" icon="i-lucide-settings" title="Webhook URL — auto-filled from Settings" description="Moved to Settings → Integration. No manual input — the global URL is used automatically.">
      <template #actions>
