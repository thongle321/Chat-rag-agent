    ...
    <UDashboardPanel id="integrations">
        ...
        <template #body>
            ...
            <div v-if="activeTab === 'zalo'">
                <div class="flex justify-center py-12" v-if="zaloLoading">
                    <ULoader />
                </div>
                <div
                    class="flex flex-col items-center justify-center py-24"
                    v-else-if="!zaloChannels.length"
                >
                    <UIcon
                        class="text-muted size-16 mb-4"
                        name="i-lucide-bot"
                    />
                    <h3 class="text-lg font-semibold mb-2">
                        No Zalo bots connected
                    </h3>
                    <p class="text-muted text-sm mb-6">Connect a Zalo Bot</p>
                    <UButton
                        icon="i-lucide-plus"
                        @click="zaloConnectModalOpen = true"
                        >Connect Zalo Bot</UButton
                    >
                </div>
                <div
                    class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4"
                ...
                >
                    <UCard
                        v-for="ch in zaloChannels"
                        :key="ch.id"
                        class="flex flex-col cursor-pointer hover:shadow-md transition"
                        @click="$router.push(`/admin/integrations/zalo/${ch.slug || ch.id}`)"
                    >
                        <div class="flex items-center gap-3 mb-4">
                            <div
                                class="flex items-center justify-center size-10 rounded-lg bg-primary/10 shrink-0"
                        ...
                        <div class="flex flex-wrap gap-2 mt-auto">
                            ...
                            <UButton
                                ...
                                variant="ghost"
                                :loading="healthChecking === ch.id"
                                @click.stop="testZaloConnection(ch)"
                                >Test</UButton
                            >
                            <UButton
                                color="error"
                                ...
                                size="xs"
                                variant="ghost"
                                @click.stop="confirmZaloDisconnect(ch)"
                                >Delete</UButton
                            >
                        </div>
                    </UCard>
    ...
    <UModal title="Connect Channel" v-model:open="connectModalOpen">
        <template #body>
            ...
            >
                ...
                <UFormField
                    hint="Must match the verification code in Facebook Developer"
                    label="Verify Token"
                    name="verify_token"
                    required
                >
    ...
    <UModal title="Connect Zalo Bot" v-model:open="zaloConnectModalOpen">
        <template #body>
            <UForm
                ...
                :state="zaloConnectState"
                @submit="handleZaloConnect"
            >
                ...
                <UAlert
                    v-if="zaloConnectError"
                    color="error"
                    variant="subtle"
                    :description="zaloConnectError"
                />
            </UForm>
        </template>
        <template #footer="{ close }">
            ...
            />
            <UButton
                form="zalo-connect-form"
                label="Connect"
                type="submit"
                :loading="zaloConnectSaving"
            />
        </template>
    </UModal>
    <UModal title="Edit Zalo Bot" v-model:open="zaloEditModalOpen">
        <template #body>
            <UForm
                class="space-y-4"
                id="zalo-edit-form"
                :schema="zaloEditSchema"
                :state="zaloEditState"
                @submit="handleZaloSave"
            >
                <UFormField
                    label="Bot Token"
                    name="bot_token"
                    ...
                    ><UInput
                        ...
                        size="sm"
                        type="password"
                        v-model="zaloEditState.bot_token"
        ...
        <template #footer="{ close }"
            ><UButton
                ...
                label="Save"
                type="submit"
                :loading="zaloEditSaving"
        /></template>
    </UModal>
    <UModal
        title="Disconnect Zalo bot"
        v-model:open="zaloDisconnectConfirmOpen"
        :description="`Disconnect ${zaloDisconnectTarget?.bot_username || 'this bot'}?`"
    >
        <template #footer="{ close }"
            ><UButton
                color="neutral"
                ...
                color="error"
                label="Disconnect"
                :loading="zaloDisconnecting"
                @click="handleZaloDisconnect"
        /></template>
    </UModal>
</template>
