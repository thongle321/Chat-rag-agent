        ...
        const { data } = await api.get("/logs/usage", {
            params: {
                ...(provider.value !== "all"
                    ...
                    : {}),
                ...(dateFrom.value
                    ? { date_from: dateFrom.value.toString().slice(0, 10) }
                    : {}),
                ...(dateTo.value
                    ? { date_to: dateTo.value.toString().slice(0, 10) }
                    : {}),
                page: page.value,
                per_page: perPage,
            },
                ...
                <UCard :ui="{ body: 'p-0' }">
                    <UTable :data="rows" :columns="columns" :loading="loading">
                        ...
                        <template #provider-cell="{ row }">
                            <UBadge
                                v-if="row.original.provider"
                                color="neutral"
                                variant="subtle"
                                size="sm"
                                >{{ row.original.provider }}</UBadge
                            >
                    ...
                    <div
                        v-if="total > perPage"
                        class="flex justify-center p-3 border-t"
                    >
                        <UPagination
                            v-model="page"
                            :total="total"
