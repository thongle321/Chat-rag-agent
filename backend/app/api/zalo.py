...
async def zalo_sync(identifier: str, user: User = current_active_user, db: AsyncSession = Depends(get_async_session)):
    ch = await get_channel_by_identifier(db, identifier)
    if not ch:
        raise HTTPException(status_code=404, detail="Channel not found")
    health = await _zalo_get_me(ch["bot_token"])
    if not health.get("ok"):
        try:
            await update_last_sync_status(db, ch["id"], "error")
            await add_sync_log(ch["bot_id"], "error", health.get("error", "")[:200], health.get("error"), channel_type="zalo")
        except Exception:
            pass
        return {"status": "error", "health": health, "detail": health.get("error", "")[:200]}

    bot_token = ch["bot_token"]
    had_webhook_url = (ch.get("webhook_url") or "").strip()

    # If webhook is configured, messages arrive in real-time. Do a fast health check without 30s getUpdates delay.
    if had_webhook_url:
        status = "success"
        detail = "Synced — webhook is active (messages arrive in real-time)"
        try:
            await update_last_sync_status(db, ch["id"], status)
            await add_sync_log(ch["bot_id"], status, detail, None, channel_type="zalo")
        except Exception:
            pass
        return {"status": status, "health": health, "detail": detail, "imported_msgs": 0}

    # Polling mode (no webhook / local dev): drain getUpdates queue with fast timeout (1s instead of 30s)
    imported_convs = 0
    imported_msgs = 0
    seen_ids: set[str] = set()
    from app.db.conversation_store import link_page_to_session, load_messages
    from app.db.conversation_store import save_messages as save_conv_messages
    from pydantic_ai.messages import ModelRequest, ModelResponse, TextPart, UserPromptPart

    offset = None
    for _ in range(10):  # safety cap 10 batches
        bot = await _with_bot(bot_token)
        try:
            kw = {"timeout": 1}
            if offset is not None:
                kw["offset"] = offset
            batch = await bot.get_update(**kw)
            if batch is None:
                batch = []
            elif not isinstance(batch, list):
                batch = [batch]
        except Exception as e:
            logger.warning("Zalo getUpdates failed: %s", e)
            batch = []
        finally:
            await _shutdown_bot(bot)
        ...
        for upd in batch:
            try:
                msg = getattr(upd, "message", None) or getattr(upd, "effective_message", None)
                if msg is None and isinstance(upd, dict):
                    r = upd.get("result") or upd
                    msg = (r.get("message") or {}) if isinstance(r, dict) else {}
                    chat = msg.get("chat", {}) if isinstance(msg, dict) else {}
                    frm = msg.get("from", {}) if isinstance(msg, dict) else {}
                    chat_id = str(chat.get("id") or frm.get("id") or "")
                    mid = str(msg.get("message_id") or "")
                    text = msg.get("text") or msg.get("caption") or ""
                    ename = r.get("event_name", "") if isinstance(r, dict) else ""
                    disp = frm.get("display_name", "") if isinstance(frm, dict) else ""
                    is_bot = frm.get("is_bot", False) if isinstance(frm, dict) else False
                else:
                    chat = getattr(msg, "chat", None)
                    frm = getattr(msg, "from_user", None) or getattr(msg, "from", None)
                    chat_id = str(getattr(chat, "id", "") or getattr(frm, "id", "") or "")
                    mid = str(getattr(msg, "message_id", "") or "")
                    text = getattr(msg, "text", "") or getattr(msg, "caption", "") or ""
                    ename = getattr(upd, "event_name", "") or ""
                    disp = getattr(frm, "display_name", "") or ""
                    is_bot = getattr(frm, "is_bot", False) or False
                if not chat_id or not text or ename == "message.unsupported.received" or is_bot:
                    continue
                ...
                if mid:
                    seen_ids.add(mid)
                try:
                    await link_page_to_session(chat_id, ch["bot_id"], username=disp or None, channel_type="zalo")
                except Exception:
                    pass
                existing = await load_messages(chat_id)
                if existing and existing[-1].__class__.__name__ == "ModelRequest":
                    last_txt = ""
                    for p in existing[-1].parts:
                        if hasattr(p, "content"):
                            last_txt = p.content
                    if last_txt == text:
                        continue
                new_msg = ModelRequest(parts=[UserPromptPart(content=text)])
                to_save = (existing + [new_msg])[-1000:]
                await save_conv_messages(chat_id, to_save)
                imported_msgs += 1
    ...
    detail = f"Synced {imported_msgs} messages via polling" if imported_msgs else "Synced — no pending messages in queue"
    status = "success"
    try:
        await update_last_sync_status(db, ch["id"], status)
        await add_sync_log(ch["bot_id"], status, detail, None, channel_type="zalo")
        ...
        pass
    return {"status": status, "health": health, "detail": detail, "imported_msgs": imported_msgs}
    try:
        await update_last_sync_status(db, ch["id"], status)
        await add_sync_log(ch["bot_id"], status, detail, None, channel_type="zalo")
    except Exception:
