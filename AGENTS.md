...
frontend/
  ...
  src/
    ...
    api/index.ts         # axios (180s timeout, JWT + 401/403 redirect interceptors, getErrorMessage) + streamChat (SSE via fetch; StreamSource/StreamHandlers)
    ...
    components/chat/ Indicator.vue / SourceLink.vue / ChatHeader.vue (title menu + auth buttons) / UserMessage.vue (bubble + hover edit) / AssistantMessage.vue (thinking + Comark + sources + followups + copy) / ChatSkeleton.vue / RenameConversationModal.vue (shared sidebar+header) / SidebarUserMenu.vue
    components/auth/ AuthCard.vue (card shell) / PasswordInput.vue (eye toggle) / LoginForm.vue + RegisterForm.vue (zod, emit submit)
    components/admin/ ConfirmModal.vue (shared delete/disconnect) / EmptyState.vue (shared empty pages) / channels/ChannelCard.vue + FacebookConnectForm/EditForm + ZaloConnectForm/EditForm / products/ProductTable.vue + ProductFormModal.vue (manual+CSV) + ShopifyCatalogModal.vue / settings/ZaloWebhookCard.vue
    pages/ index.vue (:session-id=null + clearActive), c/[id].vue (:key + replace('/') on unknown), login.vue, 404.vue, [...all].vue, admin.vue (layout),
           ...
           admin/messages/index.vue, admin/messages/[id].vue, admin/usage.vue (AI cost table: UTable + provider/date filters, USD 4dp + VND ×26000)
...
2. `Deps` + per-task agents (pydantic-ai) — analyzer routes each turn (`shopping`/`docs`/`general`, intent+filters in one temp-0 call, reused via `deps.analysis`); `app/prompts.py` owns one system prompt per task: docs builds on the default base (`Settings.context_prompt`) + catalog, `shopping`/`general` are fully standalone — no keyword routing. Tools per agent: docs→`search_documents`, shopping→`search_products`/`search_shopify_catalog`, general→none; doc catalog injected for docs only. Shopping cites via structured output (`cited_ids`, validated against tool results — no markers in prose, no card UI, plain answers); `ProcessHistory(_keep_recent)` caps history to 10 (drops leading orphan responses), `ReinjectSystemPrompt` refreshes catalog. `UsageLimits(request_limit=3)`; 120s agent + stream timeouts; provider-misconfig → 502 hint.
3. `stream_answer()` — loads history via `conversation_store`, runs `agent.run_stream`, yields `text_delta` (shopping streams `answer` via `stream_output` partials), persists citation stubs (`metadata.sources` chunk-ids on the last `ModelResponse`), saves `history + new_messages`, durable logs (`log_chat_message` user+assistant with tokens/latency/model + `log_activity chat.query` + `AIUsageLog` row with provider/model/tokens/`usage.cost` USD (Ollama Cloud fallback: `ollama_cloud_cost` static ollama.com/pricing map when cost is None; local stays NULL), guests skip all). Emits `followups` (only when `products_searched` and nothing cited).
4. Citations: only numbers actually present in answer (`\[(\d+)\]`) are surfaced; doc metadata hydrated at read-time from vector store so renames/deletes reflect immediately. Product search: hybrid RRF over the `products` collection (dense over-retrieve + `bm25_ranks`, `fuse_ranks` = official `1/(k+rank)` k=60, `PRODUCT_DISTANCE_GATE=0.70` on cosine distance — the old SQL-scan gate was cosine similarity ≥ 0.30), `category` pushed into dense `query(where=)` pre-fusion (+ Python post-filter for BM25 strays), `max_price` post-filter on chunk metadata (priceless rows excluded when gated), deterministic re-rank (in-stock → category-overlap → fusion score); display fields hydrate from the index so SQL is never scanned and the embedding-failure LIKE fallback is gone. Followup strip is bullet-only (`[•-]|\d+[.)]` + whitespace) so prices like `2 for $10?` survive.
