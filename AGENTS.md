...
2. `Deps` + per-task agents (pydantic-ai) — analyzer routes each turn (`shopping`/`docs`/`general`, intent+filters in one temp-0 call, reused via `deps.analysis`); `app/prompts/` owns one system prompt per task: docs builds on the default base (`Settings.context_prompt`) + catalog, `shopping.py`/`general.py` are fully standalone — no keyword routing. Tools per agent: docs→`search_documents`, shopping→`search_products`/`search_shopify_catalog`, general→none; doc catalog injected for docs only. Shared `[Pn]` numbering; `ProcessHistory(_keep_recent)` caps history to 10 (drops leading orphan responses), `ReinjectSystemPrompt` refreshes catalog. `UsageLimits(request_limit=3)`; 120s agent + stream timeouts; provider-misconfig → 502 hint.
3. `stream_answer()` — loads history via `conversation_store`, runs `agent.run_stream`, yields `text_delta`, persists citation stubs (`metadata.sources` chunk-ids) + cited product snapshots (`metadata.products`) on the last `ModelResponse`, saves `history + new_messages`, durable logs (`log_chat_message` user+assistant with tokens/latency/model + `log_activity chat.query`). Emits `products` (only `[P1]`-cited) + `followups` (only when `products_searched` and nothing cited, budget/category/dietary preferred).
