...
2. `Deps` + per-task agents (pydantic-ai) — analyzer routes each turn (`shopping`/`docs`/`general`, intent+filters in one temp-0 call, reused via `deps.analysis`); `app/prompts/` owns one self-contained system prompt per task (`shopping.py`, `docs.py` + catalog `build()`, `general.py`, `analyzer.py`) — no shared default prompt, no keyword routing. Tools per agent: docs→`search_documents`, shopping→`search_products`/`search_shopify_catalog`, general→none; doc catalog injected for docs only. Shared `[Pn]` numbering; `ProcessHistory(_keep_recent)` caps history to 10 (drops leading orphan responses), `ReinjectSystemPrompt` refreshes catalog. `UsageLimits(request_limit=3)`; 120s agent + stream timeouts; provider-misconfig → 502 hint.
